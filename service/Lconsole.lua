...
local function console_main_loop(stdin, print)
	print("Welcome to lyugame console")
...
skynet.start(function()
	...
	socket.start(listen_socket , function(id, addr)
		...
		socket.start(id)
        ConnectList[id] = {}
        ConnectList[id].ip = ip
        ConnectList[id].port = port
		skynet.fork(console_main_loop, id , print)
	end)

    skynet.dispatch( "lua", function(_,_, command, ...)
        local f = LUACMD[command]
        if f then 
            f(...)
        end
    end)
...
function COMMAND.abort()
    for id , info in pairs( ConnectList ) do
        socket.write(id, "lyugame is closing")
        socket.write(id, "\n")
    end
    skynet.call( ".PlayerManager", "lua", "closeall" )
end